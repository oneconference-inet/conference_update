...
const AbstractPollResults = (Component: AbstractComponent<AbstractProps>) => (props: InputProps) => {
    ...
    const answers: Array<AnswerInfo> = useMemo(() => {
        ...
        const totalSenderWeight = senderWeights ? 0 : senderWeights.reduce((previous, current) => previous + Number(current.weight), 0)
        ...
        return pollDetails.answers.map(answer => {
            ...
            // const percentage1 = totalVoters === 0 ? 0 : Math.round(answerWeight(answer.voters) / totalSenderWeight * 100);
